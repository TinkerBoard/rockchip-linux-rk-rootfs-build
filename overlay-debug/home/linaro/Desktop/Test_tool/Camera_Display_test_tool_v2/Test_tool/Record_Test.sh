...
gst-launch-1.0 v4l2src device=$CSI0 ! video/x-raw,width=640,height=480,framerate=30/1 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=640,height=480,framerate=30/1 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_OV5647_480p.avi &
var=$!
sleep 60
kill -9 $var
echo -e "$(date): Camera record Record_ov5647_480p.avi" | tee -a $ResultFile

gst-launch-1.0 v4l2src device=$CSI0 ! video/x-raw,width=1280,height=720,framerate=30/1 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=1280,height=720,framerate=30/1 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_OV5647_720p.avi &
var=$!
sleep 60
kill -9 $var
...
gst-launch-1.0 v4l2src device=$CSI0 ! video/x-raw,width=1920,height=1080 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=1920,height=1080 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_OV5647_1080p.avi &
var=$!
sleep 60
kill -9 $var
...
gst-launch-1.0 v4l2src device=$CSI1 ! video/x-raw,width=640,height=480,framerate=30/1 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=640,height=480,framerate=30/1 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_IMX219_480p.avi &
var=$!
sleep 60
kill -9 $var
echo -e "$(date): Camera record Record_IMX219_480p.avi" | tee -a $ResultFile

gst-launch-1.0 v4l2src device=$CSI1 ! video/x-raw,width=1280,height=720,framerate=30/1 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=1280,height=720,framerate=30/1 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_IMX219_720p.avi &
var=$!
sleep 60
kill -9 $var
echo -e "$(date): Camera record Record_IMX219_720p.avi" | tee -a $ResultFile

gst-launch-1.0 v4l2src device=$CSI1 ! video/x-raw,width=1920,height=1080,framerate=30/1 ! tee name=t t. ! queue ! rkximagesink sync=false t. ! queue ! videorate ! video/x-raw,width=1920,height=1080,framerate=30/1 ! mpph264enc ! queue ! h264parse ! avimux ! filesink location=/tmp/Record_IMX219_1080p.avi &
var=$!
sleep 60
kill -9 $var
